...
function category_catalog_array($category, $limit = null,
$offset = 0) {
    include("connection.php");
    $category = strtolower($category);
    try {
...
function search_catalog_array($search, $limit = null,
$offset = 0) {
    include("connection.php");
...
function genre_array($category = null) {
    ...
    try {
      ...
      if (!empty($category)) {
        ...
        $results = $db->prepare($sql . " ODER BY genre");
      }
      $results->execute();
    } catch (Exception $e) {
      echo "bad query";
    ...
    $genres = array();
    while ($row = $results->(PDO::FETCH_ASSOC)) {
